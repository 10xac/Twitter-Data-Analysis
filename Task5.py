...
st.set_page_config(page_title="FINTECH TWITTER INFORMATION", layout="wide")

display_visual= st.sidebar.selectbox('options',('Data Display','Data Visualizations'))
...
def stBarChart():
    df = loadData()
    dfCount = pd.DataFrame({'Tweet_count': df.groupby(['original_author'])['text'].count()}).reset_index()
...
def langPie():
    df = loadData()
    dfLangCount = pd.DataFrame({'Tweet_count': df.groupby(['language'])['text'].count()}).reset_index()
...
if display_visual=='Data Display':

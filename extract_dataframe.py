...
class TweetDfExtractor:
    ...
    def get_tweet_df(self, save=False) -> pd.DataFrame:
        ...
        data_dic = {'created_at': created_at, 'source': source, 'original_text': text, 'polarity': polarity,
                    'subjectivity': subjectivity, 'lang': lang, 'favorite_count': fav_count,
                    'retweet_count': retweet_count,
                    'original_author': screen_name, 'followers_count': follower_count, 'friends_count': friends_count,
                    'possibly_sensitive': sensitivity, 'hashtags': hashtags,
                    'user_mentions': mentions, 'place': location}
        df = pd.DataFrame(data=data_dic, columns=columns)
